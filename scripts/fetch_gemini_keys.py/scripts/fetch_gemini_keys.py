...
def _read_firestore_keys(id_token: str, user_id: str) -> list[str]:
    path = "/v1/projects/{}/databases/{}/documents/users/{}/apiKeys".format(
        FIREBASE_PROJECT_ID,
...
def _configured_keys() -> list[str]:
    ...
    if refresh_token:
        try:
            ...
            print(f"Loaded {len(keys)} Gemini key(s) from the Firebase vault.", file=sys.stderr)
        except Exception as exc:
            print(
                "Firebase vault unavailable; using the static Gemini secret if present "
                f"({type(exc).__name__}).",
                file=sys.stderr,
            )
    else:
        print("FIREBASE_REFRESH_TOKEN is not set; using the static Gemini secret.", file=sys.stderr)
...
def main() -> None:
    ...
    print(f"GEMINI_API_KEY={keys[0]}")
    print(f"GEMINI_API_KEYS={','.join(keys)}")
