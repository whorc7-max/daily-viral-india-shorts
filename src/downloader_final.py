...
class VideoDownloader:
    ...
    def search_openverse_images(self, scenes: list[dict] | list[str]) -> list[dict]:
        ...
        for scene in self._queries(scenes):
            try:
                ...
                resp.raise_for_status()
                result = next(
                    (item for item in resp.json().get("results", []) if item.get("thumbnail") or item.get("url")),
                    None,
                )
                if result:
                    images.append({
                        "id": f"openverse_{len(images)}",
                        "url": result.get("thumbnail") or result["url"],
    ...
    def download_image(self, image: dict, output_dir: str) -> str | None:
        path = self.download_file(image["url"], output_dir, f"img_{image['id']}", ".jpg")
        fallback_url = image.get("fallback_url")
        if not path and fallback_url and fallback_url != image["url"]:
            return self.download_file(fallback_url, output_dir, f"img_{image['id']}", ".jpg")
        return path
